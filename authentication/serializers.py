...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    default_error_messages = {
        'username': 'O nome de usuário deve conter apenas letras e espaços'}
...
class SetNewPasswordSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        try:
            ...
            print(token, 'em reset complete')
            uidb64 = attrs.get('uidb64')
            id = force_str(urlsafe_base64_decode(uidb64))
            user = User.objects.get(id=id)
            print(PasswordResetTokenGenerator().check_token(user, token), 'em reset complete')
...
class LogoutSerializer(serializers.Serializer):
    ...
    default_error_message = {
        'bad_token': 'O token expirou ou é inválido'
    }
...
class ChancePasswordSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['email', 'password', 'new_password', 'tokens']
